...
void USART2_ReportIMU(void) {
  ...
  memcpy(uart2_send_buf + 3 + sizeof(float) * 3, accel_output,
         sizeof(float) * 3);
...
void USART2_IRQHandler(void) {
  ...
  if (USART2->SR & 0x20) {
    ...
    if (uart_receive == '\n' && uart2_recv_buf[0] == '$') {
      temp_desire_l =
          (uart2_recv_buf[2] - '0') * 10000 + (uart2_recv_buf[3] - '0') * 1000 +
          (uart2_recv_buf[4] - '0') * 100 + (uart2_recv_buf[5] - '0') * 10 +
          (uart2_recv_buf[6] - '0');
      if (uart2_recv_buf[1] == '-') {
      ...
      temp_desire_r =
          (uart2_recv_buf[9] - '0') * 10000 +
          (uart2_recv_buf[10] - '0') * 1000 + (uart2_recv_buf[11] - '0') * 100 +
          (uart2_recv_buf[12] - '0') * 10 + (uart2_recv_buf[13] - '0');
      if (uart2_recv_buf[8] == '-') {
